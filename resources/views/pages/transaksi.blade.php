                    ...
                    <form method="POST" action="{{ route('transaksi.store') }}" enctype="multipart/form-data"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Kurir
                                        Pengiriman <span class="text-xs text-gray-500">(Opsional)</span></label>
                                    <input type="text" name="kurir" placeholder="Contoh: Kurir Pribadi / JNT"
                                        class="dark:bg-gray-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                                </div>

                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Plat
                                        Polisi / Resi <span class="text-xs text-gray-500">(Opsional)</span></label>
                                    <input type="text" name="no_kurir" placeholder="Contoh: P 1234 XY"
                                        class="dark:bg-gray-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                                </div>
                            </div>
                        </div>
                            ...
                            <template x-teleport="body">
                                <div x-show="modalEdit" style="display: none;"
                                    ...
                                    <div class="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8"
                                        ...
                                        x-transition:enter-end="opacity-100 scale-100">
                                        <div class="mb-6">
                                            <h4 class="mb-2 text-xl font-semibold text-gray-800 dark:text-white/90">Update
                                                Transaksi #TRX-{{ $transaksi->id_transaksi }}</h4>
                                        </div>
                                        ...
                                        <form method="POST"
                                            ...
                                            <div>
                                                <label
                                                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Update
                                                    Status</label>
                                                <select name="status" required
                                                    class="dark:bg-gray-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                                                    <option value="pending" {{ $st == 'pending' ? 'selected' : '' }}>
                                                        Pending</option>
                                                    <option value="diproses" {{ $st == 'diproses' ? 'selected' : '' }}>
                                                        Diproses (Dikemas)</option>
                                                    <option value="dikirim" {{ $st == 'dikirim' ? 'selected' : '' }}>Dalam
                                                        Pengiriman (Dikirim)</option>
                                                    <option value="selesai" {{ $st == 'selesai' ? 'selected' : '' }}>
                                                        Selesai (Tiba di Pembeli)</option>
                                                    <option value="batal" {{ $st == 'batal' ? 'selected' : '' }}>Batal
                                                    </option>
                                                </select>
                                                <p class="mt-1 text-xs text-gray-500">Peringatan: Mengubah status ke
                                                    'Selesai' akan memindahkan data ke menu Rekap.</p>
                                            </div>

                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label
                                                        class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Kurir
                                                        Pengiriman</label>
                                                    <input type="text" name="kurir" value="{{ $transaksi->kurir }}"
                                                        class="dark:bg-gray-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                                                </div>
                                                <div>
                                                    <label
                                                        class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Plat
                                                        / Resi</label>
                                                    <input type="text" name="no_kurir"
                                                        value="{{ $transaksi->no_kurir }}"
                                                        class="dark:bg-gray-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-3 mt-4 justify-end">
                                                <button @click="modalEdit = false" type="button"
                                                    class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">Batal</button>
                                                <button type="submit"
                                                    class="rounded-lg bg-yellow-500 px-4 py-2 text-sm font-medium text-white hover:bg-yellow-600">Update
                                                    Data</button>
                                            </div>
                                        </form>
                                    </div>
