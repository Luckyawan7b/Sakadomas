...
@section('content')
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('kandang.kamar', $kandang->id_kandang) }}"
                    class="inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="relative w-full sm:w-auto">
                    <select x-model="filterStatus"
                        class="w-full appearance-none rounded-lg border border-gray-300 bg-white px-4 py-2.5 pr-10 text-sm font-medium text-gray-700 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                ...
                @if(strtolower(trim($kamar->status ?? '')) === 'karantina')
                    <button type="button" class="inline-flex items-center justify-center font-medium gap-2 rounded-lg transition px-4 py-3 text-sm bg-gray-300 text-gray-500 cursor-not-allowed dark:bg-gray-700 dark:text-gray-400" title="Kamar sedang dikarantina">
        ...
        {{-- Quarantine Banner --}}
        @if(strtolower(trim($kamar->status ?? '')) === 'karantina')
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-700 dark:bg-amber-900/10">
                <div class="flex items-start gap-3">
                    <div class="mt-0.5 text-amber-500 dark:text-amber-400">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                    </div>
                    <div>
                        <h4 class="text-base font-semibold text-amber-800 dark:text-amber-400">Kamar Sedang Dikarantina!</h4>
                        <p class="text-sm text-amber-700 dark:text-amber-500 mt-1">Penambahan ternak baru ke kamar ini sementara dikunci. Buka karantina jika kamar sudah aman.</p>
                    </div>
                </div>
                <form method="POST" action="{{ route('kamar.update', $kamar->id_kamar) }}" class="flex-shrink-0">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600 transition shadow-sm">
            ...
            <div x-show="modalTambah" style="display: none;"
                ...
                <div class="no-scrollbar relative w-full max-w-[600px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8"
                    ...
                    <div class="mb-2">
                        ...
                        <p class="text-sm text-gray-500 mt-1">ke Kandang {{ $kandang->nomor_kandang }}, Kamar
                            {{ $kamar->nomor_kamar }}</p>
                    </div>
                    ...
                    <div x-data="{
                        ...
                        <div class="flex border-b border-gray-200 dark:border-gray-700 mb-6 mt-4 gap-4">
                            <button type="button" @click="tab = 'baru'"
                                ...
                                class="w-1/2 py-2.5 text-center text-sm font-medium border-b-2 transition-colors">Registrasi
                                Baru</button>
                            <button type="button" @click="tab = 'ada'"
                                :class="tab === 'ada' ? 'border-brand-500 text-brand-500' :
                                    'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200'"
                                class="w-1/2 py-2.5 text-center text-sm font-medium border-b-2 transition-colors">Pilih
                                Ternak Kosong</button>
                        </div>
                        ...
                        <form x-show="tab === 'baru'" method="POST" action="{{ route('ternak.store') }}"
                                ...
                                <div>
                                    ...
                                    <select name="id_jenis_ternak" required
                                        ...
                                        <option value="" disabled selected
                                            class="bg-white text-gray-800 dark:bg-gray-900 dark:text-white">Pilih Jenis
                                        </option>
                            ...
                            <div class="flex items-center gap-3 mt-4 justify-end">
                                <button @click="modalTambah = false" type="button"
                                    class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] sm:w-auto">Batal</button>
                                <button type="submit"
                        ...
                        <form x-show="tab === 'ada'" method="POST" :action="'/ternak/' + selectedTernakId"
                            ...
                            <div>
                                ...
                                <select x-model="selectedTernakId" @change="updateTernakData" required
                                    ...
                                    <option value="" disabled
                                        class="bg-white text-gray-800 dark:bg-gray-900 dark:text-white">Pilih Data Ternak
                                    </option>
                                    @if ($ternak_kosong->count() > 0)
                                        @foreach ($ternak_kosong as $tk)
                                            <option value="{{ $tk->id_ternak }}"
                                                class="bg-white text-gray-800 dark:bg-gray-900 dark:text-white">
                                                ID-{{ $tk->id_ternak }} ({{ $tk->jenis_ternak->jenis_ternak ?? 'Ternak' }}
                                                - {{ $tk->berat }}Kg)
                                            </option>
                                        @endforeach
                                    @else
                                        <option value="" disabled
                                            class="bg-white text-gray-800 dark:bg-gray-900 dark:text-white">Saat ini tidak
                                            ada ternak kosong</option>
                                    @endif
                                </select>
                            </div>

                            <div x-show="selectedTernakId !== ''"
                                class="mt-2 bg-gray-50 dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-sm">
                                <ul class="text-gray-700 dark:text-gray-300 space-y-1 mb-2">
                                    <li><strong>Kelamin:</strong> <span x-text="selectedTernakData.jenis_kelamin"
                                            class="capitalize"></span></li>
                                ...
                                <p class="text-xs text-brand-600 dark:text-brand-400 font-medium">*Ternak ini akan
                                    dimasukkan ke Kamar {{ $kamar->nomor_kamar }}</p>
                            </div>
                            ...
                            <div class="flex items-center gap-3 mt-4 justify-end">
                                ...
                                <button type="submit" :disabled="selectedTernakId === ''"
                                    ...
                                    class="flex w-full justify-center rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 sm:w-auto">Masukkan
                                    ke Kamar</button>
                            </div>
                        </form>
                    </div>
        ...
        <div
            class="rounded-sm border border-gray-200 bg-white shadow-default dark:border-gray-800 dark:bg-gray-900 mb-8 mt-6">
            <div
                class="py-4 px-4 md:px-6 xl:px-7.5 bg-gray-50 dark:bg-gray-800/50 flex justify-between items-center border-b border-gray-200 dark:border-gray-800">
                <h4 class="text-lg font-semibold text-black dark:text-white">
                    Daftar Ternak di Kamar Ini
                </h4>
                <span
                    class="text-sm font-medium {{ $data_ternak->count() >= $kamar->kapasitas ? 'bg-red-100 text-red-500 dark:bg-red-500/10 dark:text-red-400' : 'bg-brand-100 text-brand-500 dark:bg-brand-500/10 dark:text-brand-400' }} px-3 py-1 rounded-full">
                    ...
                    <tbody>
                        @forelse ($data_ternak as $index => $ternak)
                            <tr x-data="{
                                ...
                                <td class="py-4 px-4 xl:px-6 text-gray-800 dark:text-gray-300">
                                    <span
                                        class="font-medium capitalize">{{ $ternak->jenis_ternak->jenis_ternak ?? '-' }}</span><br>
                                ...
                                <td class="py-4 px-4 xl:px-6 text-center">
                                    <div class="flex items-center justify-center space-x-3.5">
                                        ...
                                        <button @click="modalHapus = true" type="button"
                                            class="inline-flex items-center justify-center font-medium gap-2 rounded-lg transition px-3 py-1.5 text-sm bg-red-500 text-white hover:bg-red-600">Keluar Kandang</button>
                                    </div>
                                </td>
                                ...
                                <template x-teleport="body">
                                    <div x-show="modalHapus" style="display: none;"
                                        ...
                                        <div class="relative w-full max-w-[400px] rounded-3xl bg-white p-6 dark:bg-gray-900 text-center"
                                            ...
                                            x-transition:enter-start="opacity-0 scale-95"
                                            x-transition:enter-end="opacity-100 scale-100">
                                            <h4 class="mb-2 text-xl font-semibold text-gray-800 dark:text-white/90">Keluarkan dari Kandang?</h4>
                                            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">Yakin ingin mengeluarkan
                                                ternak <strong>#ID-{{ $ternak->id_ternak }}</strong> dari kamar ini? Data ternak akan tetap ada di menu Manajemen Ternak.</p>
                                            ...
                                            <form method="POST"
                                                ...
                                                <button @click="modalHapus = false" type="button"
                                                    class="rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] sm:w-auto">Batal</button>
                                                <button type="submit"
                        ...
                        @if ($data_ternak->isNotEmpty())
                            <tr x-show="filterStatus === 'sehat' && {{ $countSehat }} === 0" style="display: none;">
                                <td colspan="5" class="py-6 text-center text-gray-500 italic">Tidak ada ternak Sehat di
                                    kamar ini.</td>
                            </tr>
                            <tr x-show="filterStatus === 'sakit' && {{ $countSakit }} === 0" style="display: none;">
                                <td colspan="5" class="py-6 text-center text-gray-500 italic">Tidak ada ternak Sakit di
                                    kamar ini.</td>
                            </tr>
                            <tr x-show="filterStatus === 'hamil' && {{ $countHamil }} === 0" style="display: none;">
                                <td colspan="5" class="py-6 text-center text-gray-500 italic">Tidak ada ternak Hamil di
                                    kamar ini.</td>
                            </tr>
                        @endif
                    </tbody>
