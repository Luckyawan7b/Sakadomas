...
<div class="min-h-screen flex flex-col md:flex-row bg-surface">
    ...
    >
        <div class="absolute inset-0 z-0">
            <img
                ...
                src="https://media.istockphoto.com/id/176133165/id/foto/seekor-domba-putih-tunggal-berdiri-di-rumput-hijau.jpg?s=612x612&w=0&k=20&c=JqqwbpZtdCtWsQdbDDy2QSyy0959573qEdURHFMhtko="
                alt=""
    ...
    <section class="w-full md:w-1/2 flex items-center justify-center p-8 md:p-16 bg-surface overflow-y-auto">
        <div class="w-full max-w-sm">
            ...
            {{-- Status: Link terkirim (SUCCESS) --}}
            @if (session('status'))
                <div
                    class="mb-8 flex items-start gap-3 p-4 rounded-xl bg-green-100 border border-green-400"
                    role="status"
                    aria-live="polite"
                >
                    <span class="material-symbols-outlined text-green-700 mt-0.5 shrink-0" style="font-variation-settings: 'FILL' 1;" aria-hidden="true">mark_email_read</span>
                    <div>
                        <p class="text-[11px] font-bold text-green-700 uppercase tracking-widest mb-1">Email Terkirim!</p>
                        <p class="text-xs text-green-800 leading-relaxed">{{ session('status') }}</p>
                    </div>
                </div>
            @endif

            {{-- Status: Error (GAGAL MENGIRIM / EMAIL TIDAK ADA) --}}
            @if ($errors->any())
                <div
                    class="mb-8 flex items-start gap-3 p-4 rounded-xl bg-red-100 border border-red-400"
                    role="alert"
                >
                    <span class="material-symbols-outlined text-red-700 mt-0.5 shrink-0" style="font-variation-settings: 'FILL' 1;" aria-hidden="true">error</span>
                    <div>
                        <p class="text-[11px] font-bold text-red-700 uppercase tracking-widest mb-1">Peringatan</p>
                        <p class="text-xs text-red-700 leading-relaxed">{{ $errors->first() }}</p>
                    </div>
                </div>
            @endif
            ...
            <aside class="mt-16 p-4 rounded-xl bg-primary-container/10 border border-primary-container/20 flex items-start gap-4" aria-label="Informasi keamanan">
                ...
                <div>
                    ...
                    <p class="text-xs text-on-surface-variant leading-relaxed">
                        Tautan pemulihan hanya berlaku selama <strong>60 menit</strong>. Pastikan email Anda aktif.
                    </p>
                </div>
            </aside>
