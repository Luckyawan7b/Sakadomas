...
<main class="pt-24 flex-1 bg-surface text-on-surface"
    ...
    <header class="relative bg-surface-container-low pt-12 pb-16 px-8 overflow-hidden">
        <div class="absolute inset-0 opacity-10 pointer-events-none" style="background-image: url('/images/katalog.jpg'); background-size: cover; background-position: center;"></div>
        <div class="max-w-7xl mx-auto relative z-10">
            <h1 class="text-4xl md:text-5xl font-bold font-headline text-primary mb-4 tracking-tight leading-tight">Pengaturan Akun</h1>
            <p class="text-on-surface-variant max-w-xl text-lg">Kelola informasi personal dan preferensi akun Anda di sini.</p>
        </div>
    </header>
    ...
    <section class="max-w-7xl mx-auto px-8 -mt-8 mb-24 relative z-10 flex flex-col lg:flex-row gap-8">
        ...
        <aside class="w-full lg:w-80 shrink-0">
            <div class="bg-white rounded-3xl shadow-[0_20px_50px_rgba(61,103,0,0.04)] border border-surface-container-high p-4 flex flex-col gap-2 sticky top-32">

                <a href="{{ route('profile') }}" class="flex items-center gap-4 px-6 py-4 rounded-2xl bg-primary/10 text-primary font-bold transition-all">
                    <span class="material-symbols-outlined">person</span> Profil Saya
                </a>

                <a href="{{ route('transaksi.riwayat') }}" class="flex items-center gap-4 px-6 py-4 rounded-2xl text-on-surface-variant hover:bg-surface-container-low hover:text-primary font-bold transition-all">
                    <span class="material-symbols-outlined">receipt_long</span> Riwayat Pemesanan
                </a>

                <a href="{{ route('kunjungan.riwayat') }}" class="flex items-center gap-4 px-6 py-4 rounded-2xl text-on-surface-variant hover:bg-surface-container-low hover:text-primary font-bold transition-all">
        ...
        <div class="flex-1 space-y-8">
            ...
            <div class="bg-white rounded-3xl shadow-[0_20px_50px_rgba(61,103,0,0.04)] border border-surface-container-high p-8 flex flex-col sm:flex-row items-center sm:items-start gap-8">
                <div class="relative group">
                    <div class="w-32 h-32 rounded-full overflow-hidden border-4 border-surface shadow-lg">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->nama) }}&background=3d6700&color=fff&size=200" alt="Profile Picture" class="w-full h-full object-cover">
                    </div>
                </div>
                <div class="flex flex-col items-center sm:items-start flex-1 text-center sm:text-left mt-2">
                    ...
                    <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-extrabold tracking-wide uppercase bg-primary/10 text-primary border border-primary/20">
            ...
            <div class="bg-white rounded-3xl shadow-[0_20px_50px_rgba(61,103,0,0.04)] border border-surface-container-high overflow-hidden">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-1.5 font-label">Nama Lengkap</p>
                        <p class="text-base font-bold text-on-surface">{{ Auth::user()->nama ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-1.5 font-label">Username</p>
                        <p class="text-base font-bold text-on-surface">{{ '@' . Auth::user()->username }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-1.5 font-label">Alamat Email</p>
                        <p class="text-base font-bold text-on-surface flex items-center gap-2">{{ Auth::user()->email ?? '-' }} @if(Auth::user()->email)<span class="material-symbols-outlined text-status-success text-[16px]" title="Email Terdaftar">check_circle</span>@endif</p>
                    </div>
                    <div>
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-1.5 font-label">Nomor Handphone</p>
                        <p class="text-base font-bold text-on-surface">{{ Auth::user()->no_hp ?? '-' }}</p>
                    </div>
                    <div class="md:col-span-2">
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-1.5 font-label">Alamat Lengkap</p>
                        <p class="text-base font-bold text-on-surface leading-relaxed max-w-2xl">{{ Auth::user()->alamat ?? '-' }}</p>
            ...
            <div class="bg-white rounded-3xl shadow-[0_20px_50px_rgba(61,103,0,0.04)] border border-surface-container-high overflow-hidden">
                <div class="px-8 py-6 border-b border-surface-container-high bg-surface-container-lowest">
                    <h3 class="text-xl font-extrabold font-headline text-on-surface flex items-center gap-2"><span class="material-symbols-outlined text-primary">security</span> Keamanan Akun</h3>
                </div>
    ...
    <template x-teleport="body">
        <div x-show="modalEditProfile" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-on-surface/40 backdrop-blur-sm p-4" @click.self="modalEditProfile = false">
            <div x-show="modalEditProfile" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div class="px-8 py-6 border-b border-surface-container-high flex justify-between items-center bg-surface-container-lowest shrink-0">
                    ...
                    <button type="button" @click="modalEditProfile = false" class="w-8 h-8 flex items-center justify-center rounded-full bg-surface-container-low text-on-surface-variant hover:bg-surface-variant transition-all">
                ...
                <form method="POST" action="{{ route('profile.update') }}" class="flex flex-col overflow-hidden">
                    ...
                    <div class="p-8 overflow-y-auto no-scrollbar space-y-6">
                        @if ($errors->any() && !$errors->has('password_lama') && !$errors->has('password_baru'))
                            <div class="rounded-xl bg-error-container p-4 text-sm text-error font-medium border border-error/20">
                                {{ $errors->first() }}
                            </div>
                        @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="locationPickerProfile(window.locationDataProfile)">
                            <div>
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Nama Lengkap</label>
                                <input type="text" name="nama" value="{{ old('nama', Auth::user()->nama) }}" required class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Username</label>
                                <input type="text" name="username" value="{{ old('username', Auth::user()->username) }}" required class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Alamat Email</label>
                                <input type="email" name="email" value="{{ old('email', Auth::user()->email) }}" class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Nomor Handphone</label>
                                <input type="tel" name="no_hp" value="{{ old('no_hp', Auth::user()->no_hp) }}" required class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none">
                            </div>

                            {{-- Kecamatan Autocomplete --}}
                            <div class="relative">
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Kecamatan</label>
                                <div class="relative" @click.away="openKecamatan = false">
                                    <input type="hidden" name="id_kecamatan" x-model="selectedKecamatanId">
                                    <input type="text" x-model="searchKecamatan" @focus="openKecamatan = true" @input="openKecamatan = true; selectedKecamatanId = ''; selectedDesaId = ''; searchDesa = '';" placeholder="Cari Kecamatan..." autocomplete="off" required class="w-full px-5 py-3.5 pr-10 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none">
                                    <span class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-outline">search</span>

                                    <ul x-show="openKecamatan" x-transition class="absolute z-50 w-full bg-white border border-surface-container-high rounded-xl mt-1 shadow-lg max-h-60 overflow-y-auto" style="display: none;">
                                        <template x-for="kec in filteredKecamatan" :key="kec.id_kecamatan">
                                            <li @click="selectKecamatan(kec)" class="px-4 py-3 hover:bg-surface-container-low cursor-pointer text-on-surface text-sm transition-colors font-medium" x-text="kec.nama_kecamatan"></li>
                                        </template>
                            ...
                            <div class="relative">
                                ...
                                <div class="relative" @click.away="openDesa = false">
                                    ...
                                    <input type="text" x-model="searchDesa" @focus="openDesa = true" @input="openDesa = true; selectedDesaId = '';" placeholder="Cari Desa..." autocomplete="off" :disabled="!selectedKecamatanId" required class="w-full px-5 py-3.5 pr-10 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none disabled:opacity-50 disabled:cursor-not-allowed">
                                    <span class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-outline">search</span>

                                    <ul x-show="openDesa && selectedKecamatanId" x-transition class="absolute z-50 w-full bg-white border border-surface-container-high rounded-xl mt-1 shadow-lg max-h-60 overflow-y-auto" style="display: none;">
                                        <template x-for="d in filteredDesa" :key="d.id_desa">
                                            <li @click="selectDesa(d)" class="px-4 py-3 hover:bg-surface-container-low cursor-pointer text-on-surface text-sm transition-colors font-medium" x-text="d.nama_desa"></li>
                                        </template>
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-bold text-on-surface mb-2 font-label">Alamat Lengkap</label>
                                <textarea name="alamat" required class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none resize-none" rows="3">{{ old('alamat', Auth::user()->alamat) }}</textarea>
                            </div>
                        </div>
                    </div>
    ...
    <template x-teleport="body">
        <div x-show="modalGantiPassword" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-on-surface/40 backdrop-blur-sm p-4" @click.self="modalGantiPassword = false">
            <div x-show="modalGantiPassword" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                <div class="px-8 py-6 border-b border-surface-container-high flex justify-between items-center bg-surface-container-lowest">
                    <h3 class="text-xl font-extrabold font-headline text-on-surface">Ganti Password</h3>
                    <button type="button" @click="modalGantiPassword = false" class="w-8 h-8 flex items-center justify-center rounded-full bg-surface-container-low text-on-surface-variant hover:bg-surface-variant transition-all">
                        <span class="material-symbols-outlined text-[20px]">close</span>
                    </button>
                </div>
                ...
                <form method="POST" action="{{ route('profile.password') }}" class="flex flex-col">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-on-surface mb-2 font-label">Password Saat Ini</label>
                            <input type="password" name="password_lama" required class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none" placeholder="Masukkan password lama">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-on-surface mb-2 font-label">Password Baru</label>
                            <input type="password" name="password_baru" required minlength="6" class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none" placeholder="Buat password baru">
                            <p class="text-[11px] text-on-surface-variant font-medium mt-2">Minimal 6 karakter, mencakup huruf besar, huruf kecil, dan angka.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-on-surface mb-2 font-label">Konfirmasi Password Baru</label>
                            <input type="password" name="password_baru_confirmation" required minlength="6" class="w-full px-5 py-3.5 rounded-xl bg-surface-container-low border-none focus:ring-2 focus:ring-primary/50 font-bold text-on-surface outline-none" placeholder="Ketik ulang password baru">
                        </div>
                    </div>
