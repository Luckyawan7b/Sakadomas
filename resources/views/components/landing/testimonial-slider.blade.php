...
@php
    ...
    $items = $testimonials ?? [
        [
            'quote' =>
                'Domba dari Smart-Saka sangat gemuk dan sehat. Kami sudah 3 tahun berturut-turut membeli di sini untuk kebutuhan kurban. Sangat puas dan tidak akan pindah ke tempat lain!',
            'name' => 'Daniel M.',
            'role' => 'Pelanggan Tetap • Surabaya',
            'image' => asset('images/landing/testi-1.jpg'),
        ],
        [
            'quote' =>
                'Layanan luar biasa! Dari proses pemesanan, pengiriman sampai hewan tiba, semuanya profesional. Kambing Etawa yang saya beli produksi susunya benar-benar tinggi. Highly recommended!',
            'name' => 'Siti Rahayu',
            'role' => 'Usaha Susu Kambing • Malang',
            'image' => asset('images/landing/testi-2.jpg'),
        ],
        [
            'quote' =>
                'Saya beli 10 ekor domba Crosstexel untuk investasi ternak. Dalam 6 bulan hasilnya melebihi ekspektasi. Tim Smart-Saka juga sangat responsif dalam memberikan pendampingan.',
            'name' => 'Budi Prasetyo',
            'role' => 'Peternak Mitra • Jember',
            'image' => asset('images/landing/testi-3.jpg'),
        ],
    ];
    $total = count($items);
@endphp
...
<div x-data="{
    ...
    goto(i) { this.current = i;
        this.startAuto(); }
}" x-init="startAuto()" @keydown.arrow-right.window="next()"
    @keydown.arrow-left.window="prev()" class="w-full" role="region" aria-label="Slider testimoni pelanggan">
    {{-- Slide container --}}
    <div class="relative overflow-hidden">
        <div class="flex transition-transform duration-500 ease-[cubic-bezier(.4,0,.2,1)]"
                    ...
                    <div class="bg-white rounded-3xl p-8 shadow-sm border border-olive-100">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-olive-200 shrink-0">
                                <img src="{{ $testi['image'] }}" alt="Foto {{ $testi['name'] }}"
                                    class="w-full h-full object-cover" loading="lazy" width="48" height="48">
                            </div>
                            <div>
        ...
        <div class="flex gap-2" role="tablist" aria-label="Pilih slide testimoni">
            @foreach ($items as $i => $testi)
                <button @click="goto({{ $i }})"
                    ...
                    role="tab" :aria-selected="current === {{ $i }}"
                    aria-label="Slide {{ $i + 1 }}: {{ $testi['name'] }}"></button>
            @endforeach
        </div>
